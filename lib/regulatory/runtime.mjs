...
export const citationSchema = z
	...
	.superRefine((citation, context) => {
		...
		if (
			...
			(typeof citation.normalizedValue !== "number" ||
		...
		if (
			...
			(citation.transformation === null)
		) {
			context.addIssue({
				code: "custom",
				message: "Normalized values and transformations must be declared together",
				path: ["transformation"],
			});
		}
	});
		...
		competingOccurrences: z
			.array(
				z
					.object({
						rawRowId: idSchema,
						fieldCitationIds: z.record(idSchema, z.array(idSchema).min(1)),
					})
					.strict(),
			)
			.min(2),
...
function transformCitationToken(citation) {
	const token = normalizeSourceText(citation.extractedToken);
	if (citation.transformation === "decimal parse") {
		invariant(
			/^[+-]?(?:\d+(?:\.\d+)?|\.\d+)$/.test(token),
			`Citation ${citation.id} token cannot be parsed by decimal parse`,
		);
		const value = Number(token);
		invariant(
			Number.isFinite(value),
			`Citation ${citation.id} decimal parse produced a non-finite value`,
		);
		return value;
	}
	if (citation.transformation === "text identity") {
		return token;
	}
	throw new Error(
		`Citation ${citation.id} uses unsupported normalization transformation ${citation.transformation}`,
	);
}

function assertCitationNormalization(citation) {
	if (citation.normalizedValue === undefined) return;
	const transformed = transformCitationToken(citation);
	invariant(
		sameScalar(citation.normalizedValue, transformed),
		`Citation ${citation.id} transformation ${citation.transformation} produced ${JSON.stringify(transformed)}, not normalized value ${JSON.stringify(citation.normalizedValue)}`,
	);
	if (typeof transformed === "number") {
		invariant(
			citation.normalizedNumericValue === transformed,
			`Citation ${citation.id} numeric transformation requires a matching normalizedNumericValue`,
		);
	} else {
		invariant(
			citation.normalizedNumericValue === undefined && citation.unit === null,
			`Citation ${citation.id} text identity requires no numeric mirror or unit`,
		);
	}
}
...
async function readBuiltInCitationText(source, bytes, citations) {
	if (source.mediaType.startsWith("text/")) {
		const pages = Buffer.from(bytes).toString("utf8").split("\f");
		return Object.fromEntries(
			citations.map((citation) => [
				citation.id,
				pages[citation.physicalPage - 1] ?? "",
			]),
		);
	}
	if (source.mediaType !== "application/pdf") return undefined;
	...
	try {
		...
		for (const citation of citations) {
			const pageCitations = citationsByPage.get(citation.physicalPage) ?? [];
			pageCitations.push(citation);
			citationsByPage.set(citation.physicalPage, pageCitations);
		}
		const extracted = {};
		for (const [physicalPage, pageCitations] of citationsByPage) {
			const pageMetadata = source.pages.find(
				(page) => page.physicalPage === physicalPage,
			);
			invariant(pageMetadata, `Source ${source.id} has no metadata for page ${physicalPage}`);
			const page = await pdf.getPage(physicalPage);
			const viewport = page.getViewport({
				scale: 1,
				rotation: pageMetadata.rotationDegrees,
			});
			...
			const items = content.items
				...
				.map((item) => {
					...
					const points = [
						...
						[transform[4] + advance.x, transform[5] + advance.y],
						[transform[4] - rise.x, transform[5] - rise.y],
						[
							transform[4] + advance.x - rise.x,
							transform[5] + advance.y - rise.y,
						],
					];
					...
					return {
						...
						rect: {
							...
							width: (Math.max(...xValues) - left) / viewport.width,
							height: (Math.max(...yValues) - top) / viewport.height,
						},
					};
				});
			for (const citation of pageCitations) {
				extracted[citation.id] = items
					.filter((item) => intersects(citation.rect, item.rect))
					.map(({ text }) => text)
					.join(" ");
			}
		}
...
export async function verifySourceLocks(sourcesInput, repositoryRoot, options = {}) {
	const { locks } = await verifyBundledSources(
		sourcesInput,
		repositoryRoot,
		options,
		[],
...
async function verifyBundledSources(
	sourcesInput,
	repositoryRoot,
	options,
	citations,
		...
		bundledSources
			.map(async (source) => {
				...
				const sourceCitations = citations.filter(
					(citation) => citation.sourceDocumentId === source.id,
				);
				const citationText = sourceCitations.length
					? await readBuiltInCitationText(source, bytes, sourceCitations)
					: {};
				if (sourceCitations.length && citationText === undefined) {
					throw new Error(
						`Bundled source ${source.id} media type ${source.mediaType} requires a citation-text reader`,
					);
				}
				return {
					lock: {
	...
	for (const { citationText } of verified) {
		for (const [citationId, text] of Object.entries(citationText)) {
			citationTextById.set(citationId, normalizeSourceText(text));
		}
	}
	return {
		locks: verified
...
export async function checkEvidenceStudy(input, options) {
	...
	const { locks: sourceLocks, citationTextById } = await verifyBundledSources(
		study.sources,
	...
	for (const citation of study.citations) {
		...
		if (source.redistribution.decision === "bundled") {
			const sourceText = citationTextById.get(citation.id);
			invariant(
				sourceText !== undefined,
				`Citation ${citation.id} has no extraction from locked source ${source.id}`,
			);
			invariant(
				sourceText.includes(normalizeSourceText(citation.extractedToken)),
				`Citation ${citation.id} transcription is absent from locked source ${source.id}`,
			);
			for (const requiredToken of citation.requiredTokens) {
				invariant(
					sourceText.includes(normalizeSourceText(requiredToken)),
					`Citation ${citation.id} required token ${requiredToken} is absent from locked source ${source.id}`,
				);
			}
		}
		assertCitationNormalization(citation);
		...
		} else if (citation.regionKind === "equation" && citation.parentCitationId) {
			invariant(
				parent?.regionKind === "figure",
				`Equation citation ${citation.id} requires a figure parent`,
			);
		} else {
	...
	const overrideFieldCitationIdsByRawRowId = new Map();
	for (const override of study.overrides) {
		for (const occurrence of override.competingOccurrences) {
			invariant(
				!overrideFieldCitationIdsByRawRowId.has(occurrence.rawRowId),
				`Raw row ${occurrence.rawRowId} belongs to multiple override occurrences`,
			);
			overrideFieldCitationIdsByRawRowId.set(
				occurrence.rawRowId,
				occurrence.fieldCitationIds,
			);
		}
	}
	...
	function assertRowFieldSupport(label, row) {
		...
		const occurrenceFieldCitationIds =
			label === "Raw row"
				? overrideFieldCitationIdsByRawRowId.get(row.id)
				: row.sourceRowIds
						.map((rawRowId) => overrideFieldCitationIdsByRawRowId.get(rawRowId))
						.find(Boolean);
		...
		for (const fieldId of study.coverage.fieldIds) {
			...
			if (value?.provenance === "direct-source") {
				...
				const supportingCells = (occurrenceFieldCitationIds?.[fieldId] ?? value.citationIds)
			...
			if (value?.provenance === "interpolated") {
				for (const citationId of occurrenceFieldCitationIds?.[fieldId] ?? value.citationIds) {
					const citation = citationById.get(citationId);
	...
	for (const key of [...expectedRowKeys].sort(compare)) {
		...
		if (rawRows.length === 1) {
			...
			const occurrenceCitationIds = override.competingOccurrences.flatMap(
				({ fieldCitationIds }) => Object.values(fieldCitationIds).flat(),
			);
			...
			for (const occurrence of override.competingOccurrences) {
				...
				invariant(
					row,
					`Override ${override.id} references missing raw row ${occurrence.rawRowId}`,
				);
				const fieldIds = Object.keys(occurrence.fieldCitationIds).sort(compare);
				const evidenceFieldIds = study.coverage.fieldIds
					.filter((fieldId) => {
						const value = valueByCoverage.get(
							valueKey(row.optionId, row.hazardId, fieldId),
						);
						return ["direct-source", "interpolated"].includes(value?.provenance);
					})
					.sort(compare);
				invariant(
					JSON.stringify(fieldIds) === JSON.stringify(evidenceFieldIds),
					`Override ${override.id} occurrence ${occurrence.rawRowId} field evidence differs`,
				);
				for (const fieldId of evidenceFieldIds) {
					const value = valueByCoverage.get(valueKey(row.optionId, row.hazardId, fieldId));
					const citationIds = occurrence.fieldCitationIds[fieldId];
					assertUniqueStrings(
						`override occurrence ${occurrence.rawRowId} field ${fieldId}`,
						citationIds,
					);
					invariant(
						(value.provenance === "direct-source" && citationIds.length === 1) ||
							(value.provenance === "interpolated" &&
								citationIds.length === value.citationIds.length),
						`Override ${override.id} occurrence ${occurrence.rawRowId} field ${fieldId} has incomplete field evidence`,
					);
					for (const citationId of citationIds) {
						...
						invariant(
							citation?.regionKind === "cell" &&
								(value.provenance !== "direct-source" ||
									directCellMatchesRowField(citation, value, row.fields[fieldId])),
							`Override ${override.id} occurrence ${occurrence.rawRowId} field ${fieldId} has invalid cell evidence`,
						);
