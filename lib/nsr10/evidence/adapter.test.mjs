...
describe("NSR-10 generic evidence adapter", () => {
	...
	it("is installed by convention and emits byte-identical LF reports", () => {
		...
		const studies = Object.fromEntries(
			aggregate.studies.map((study) => [study.studyId, study]),
		);
		...
		expect(studies["framework-fixture"]).toMatchObject({ coverage: { expectedRows: 1 } });
		expect(studies.nsr10).toMatchObject({
			rawRows: { count: 1_124 },
