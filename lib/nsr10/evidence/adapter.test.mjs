...
describe("NSR-10 generic evidence adapter", () => {
	...
	it("is installed by convention and emits byte-identical LF reports", () => {
		...
		expect(aggregate.installedStudies).toEqual(
			expect.arrayContaining(["framework-fixture", "manizales-microzonation", "nsr10"]),
		);
		...
		expect(aggregate.studies.find(({ studyId }) => studyId === "framework-fixture")).toMatchObject({
			studyId: "framework-fixture", coverage: { expectedRows: 1 },
		});
		expect(aggregate.studies.find(({ studyId }) => studyId === "nsr10")).toMatchObject({
			studyId: "nsr10", rawRows: { count: 1_124 }, canonicalRows: { count: 1_123 },
			citations: { cell: 4_492, clause: 8 }, uncoveredValues: [],
		});
		expect(aggregate.studies.find(({ studyId }) => studyId === "manizales-microzonation")).toMatchObject({
			studyId: "manizales-microzonation", coverage: { expectedRows: 1, expectedValues: 5 }, uncoveredValues: [],
		});
	}, 30_000);
});
