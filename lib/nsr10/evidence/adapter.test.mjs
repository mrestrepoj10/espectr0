...
describe("NSR-10 generic evidence adapter", () => {
	...
	it("is installed by convention and emits byte-identical LF reports", () => {
		...
		expect(aggregate.studies.map(({ studyId }) => studyId)).toEqual(
		...
		expect(aggregate.studies.find(({ studyId }) => studyId === "ccp14-seismic-research")).toMatchObject({
		...
		expect(aggregate.studies.find(({ studyId }) => studyId === "medellin-microzonation")).toMatchObject({
		...
		expect(aggregate.studies.find(({ studyId }) => studyId === "cali-microzonation")).toMatchObject({
